ent-ClothingHeadHatHairFlowerWhite = 纯白发饰花

ent-ClothingHeadHatHairFlowerBlue = 指挥蓝发饰花

ent-ClothingHeadHatHairFlowerLightBlue = 医疗浅蓝发饰花

ent-ClothingHeadHatHairFlowerLime = 服务青柠发饰花

ent-ClothingHeadHatHairFlowerYellow = 工程黄发饰花

ent-ClothingHeadHatHairFlowerRed = 安保红发饰花

ent-ClothingHeadHatHairFlowerPink = 科研粉发饰花

ent-ClothingHeadHatHairFlowerBrown = 货运棕发饰花

ent-ClothingHeadHatHairFlowerOrange = 化学橙发饰花

ent-ClothingHeadHatHairFlowerGreen = 病毒学绿发饰花

ent-ClothingHeadHatHairFlowerTeal = 大气青发饰花

ent-ClothingHeadHatHairFlowerPurple = 打捞紫发饰花

ent-ClothingHeadHatHairFlowerGrey = 访客灰发饰花 # Starlight rename to visitor since we don't have passengers

ent-ClothingHeadHatHairFlowerBlack = 虚空黑发饰花

ent-ClothingHeadHatHairFlowerGold = 海盗金发饰花

ent-SpawnPointGhostWizardDuelistBase = 幽灵角色出生点

ent-SpawnPointGhostWizardDuelistRed = 红巫师决斗者出生点

ent-SpawnPointGhostWizardDuelistBlue = 蓝巫师决斗者出生点

ent-SpawnPointGhostWizardDuelistYellow = 黄巫师决斗者出生点

ent-SpawnPointGhostWizardDuelistGreen = 绿巫师决斗者出生点

ent-SpawnPointGhostWizardDuelistOrange = 橙巫师决斗者出生点

ent-BladeServerFrame = 刀片服务器框架

ent-ResearchAndDevelopmentBladeServer = 研发刀片服务器
    .desc = 储存着空间站科学家们的集体知识。摧毁它会让他们一夜退回石器时代。你不会想这么做的，对吧？

ent-CrewMonitoringBladeServer = 船员监控刀片服务器
    .desc = 接收并转发空间站上所有启用中制服传感器的状态。

ent-BasePlayingCard = 扑克牌

ent-BasePlayingCardDeck = 一副扑克牌
    .desc = 等待轮到你时，可以欣赏这些设计鲜艳、色彩缤纷的纸牌。

ent-BasePlayingCardDeckBox = 扑克牌盒
    .desc = 一个用来装一堆小卡纸片的小纸盒。刺激。

ent-PlayingCardDynamic = 扑克牌

ent-PlayingCardDeckDynamic = 一副扑克牌
    .desc = 图案鲜活、色彩缤纷的纸牌，等候出牌时也能赏心悦目。

ent-PlayingCardHandDynamic = 一手牌

ent-GoFishCardDeckBox = 钓鱼牌盒
    .desc = 一个用来装一叠小纸片的小纸盒。真刺激。

ent-GoFishCardDeck = 一副扑克牌
    .desc = 图案鲜明、色彩艳丽的卡牌，让你等候出牌时也能大饱眼福。

ent-PlayingCardDeckBoxBlack = 扑克牌盒
    .desc = 一个小纸盒，用来装一叠小卡纸片。真刺激。

ent-PlayingCardDeckBlack = 一副扑克牌
    .desc = 图案鲜活、色彩缤纷的纸牌，等候出牌时也能赏心悦目。

ent-PlayingCardDeckBoxSyndicate = 扑克牌盒
    .desc = 一个小纸盒，用来装一叠小卡纸片。真刺激。

ent-PlayingCardDeckSyndicate = 一副扑克牌
    .desc = 图案鲜活、色彩缤纷的纸牌，等候出牌时也能赏心悦目。

ent-PlayingCardDeckBoxNanotrasen = 扑克牌盒
    .desc = 一个小纸盒，用来装一叠小卡纸片。真刺激。

ent-PlayingCardDeckNanotrasen = 一副扑克牌
    .desc = 图案鲜活、色彩缤纷的纸牌，等候出牌时也能赏心悦目。

ent-TarotCardDeckBox = 大阿卡纳塔罗牌盒
    .desc = 一个装有一套占卜塔罗牌大阿卡纳牌的小纸盒。

ent-TarotCardDeck = 一副牌
    .desc = 图案与色彩鲜艳夺目的纸牌，让你在等待轮到自己时也能赏心悦目。

ent-WeaponNinjaChainSickle = 锁镰 # Starlight
    .desc = 一把连着蛛丝锁链的镰刀。蜘蛛氏族即使在窃取未来之时，也仍践行着古老之道。

ent-BaseResearchAndDevelopmentServer = 抽象研发服务器
